...
from ...core.models import Character, Group, Voice
...
def make_tab(parent, context: AppContext):
    ...
    broadcast_list_frame = ttk.Frame(broadcast_frame)
    broadcast_list_frame.grid(row=1, column=0, sticky="nsew", padx=8, pady=(2, 4))
    broadcast_list_frame.columnconfigure(0, weight=1)
    broadcast_list_frame.rowconfigure(0, weight=1)
    broadcast_list = tk.Listbox(
        broadcast_list_frame,
        height=10,
        background=SLATE_BG,
        foreground=LIGHT_TEXT,
        selectbackground=ACCENT_CYAN,
    ...
    broadcast_list.grid(row=0, column=0, sticky="nsew")
    broadcast_list_scroll = ttk.Scrollbar(broadcast_list_frame, orient="vertical", command=broadcast_list.yview)
    broadcast_list_scroll.grid(row=0, column=1, sticky="ns")
    broadcast_list.configure(yscrollcommand=broadcast_list_scroll.set)

    ttk.Label(broadcast_frame, text="Transcript").grid(row=2, column=0, sticky="w", padx=8)
    transcript_container = ttk.Frame(broadcast_frame)
    transcript_container.grid(row=3, column=0, sticky="nsew", padx=8, pady=(0, 8))
    ...
    transcript_scroll = ttk.Scrollbar(transcript_container, orient="vertical", command=transcript_text.yview)
    ...
    def _populate_broadcast_transcript(match_voice_ids: set[str] | None):
        transcript_text.configure(state="normal")
        transcript_text.delete("1.0", "end")
        broadcast = project.broadcasts.get(current_broadcast_id) if current_broadcast_id else None
        if not broadcast:
        ...
        for line in broadcast.lines:
            ...
            speaker = voice.name if voice and voice.name else line.voice_id or "Unknown"
            marker = " <-" if match_voice_ids and line.voice_id in match_voice_ids else ""
            transcript_text.insert("end", f"{speaker}: {line.text or ''}{marker}\n")
        transcript_text.configure(state="disabled")
    ...
    def _refresh_broadcast_list():
        ...
        for broadcast in ordered:
            broadcast_list.insert("end", broadcast.title or broadcast.id)
        if not broadcast_ids:
            current_broadcast_id = None
            _populate_broadcast_transcript(match_voice_ids)
            return
        if current_broadcast_id not in broadcast_ids:
            current_broadcast_id = broadcast_ids[0]
        broadcast_list.selection_clear(0, "end")
        idx = broadcast_ids.index(current_broadcast_id)
        broadcast_list.selection_set(idx)
        broadcast_list.see(idx)
        _populate_broadcast_transcript(match_voice_ids)

    def _on_broadcast_selected(event=None):
        nonlocal current_broadcast_id
        match_voice_ids, _ = _resolve_broadcast_filter()
        selection = broadcast_list.curselection()
        if not selection:
            current_broadcast_id = None
            _populate_broadcast_transcript(match_voice_ids)
            return
        current_broadcast_id = broadcast_ids[selection[0]]
        _populate_broadcast_transcript(match_voice_ids)
    ...
    broadcast_list.bind("<<ListboxSelect>>", _on_broadcast_selected)
    refresh_all()
    return frame
