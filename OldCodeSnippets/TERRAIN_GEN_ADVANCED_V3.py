...
DEFAULT_LAYER_CONFIG = {
    "terrain": {
        ...
        "layers": [
            ...
            {"name": "Sand",        "key": "sand",        "color": "#d2c8a0"},
        ]
    },
    "vegetation": {
        "seed": 5,
        "layers": [
            ...
            {"name": "None",     "key": "None",      "color": "#000000"},
        ]
    },
    "water": {
        "seed": 7,
...
# ==== RIVER / PATH / SHORE DEFAULTS ====
# tuned to "natural-ish"
RIV_SCALE_DEFAULT = 100.0        # feature size
RIV_BAND_DEFAULT  = 0.013        # base thickness
RIV_MINF_DEFAULT  = 0.7          # min thickness multiplier
RIV_MAXF_DEFAULT  = 1.5          # max thickness multiplier
RIV_ROT_DEFAULT   = 22.0         # degrees
RIV_CURVE_DEFAULT = 14.0         # domain warp strength
RIV_AMOUNT_DEFAULT = 1.0         # overall coverage multiplier

PATH_SCALE_DEFAULT = 60.0
PATH_BAND_DEFAULT  = 0.011
PATH_ROT_DEFAULT   = 0.0
PATH_CURVE_DEFAULT = 8.0

SHORE_CHANCE_DEFAULT = 0.28
...
class TerrainGeneratorApp(tk.Tk):
    def __init__(self):
        ...
        # RIVER/PATH/SHORE vars
        self.riv_scale_var    = tk.DoubleVar(value=RIV_SCALE_DEFAULT)
        self.riv_band_var     = tk.DoubleVar(value=RIV_BAND_DEFAULT)
        self.riv_minf_var     = tk.DoubleVar(value=RIV_MINF_DEFAULT)
        self.riv_maxf_var     = tk.DoubleVar(value=RIV_MAXF_DEFAULT)
        self.riv_rot_var      = tk.DoubleVar(value=RIV_ROT_DEFAULT)
        self.riv_curve_var    = tk.DoubleVar(value=RIV_CURVE_DEFAULT)
        self.riv_amount_var   = tk.DoubleVar(value=RIV_AMOUNT_DEFAULT)

        self.path_scale_var   = tk.DoubleVar(value=PATH_SCALE_DEFAULT)
        self.path_band_var    = tk.DoubleVar(value=PATH_BAND_DEFAULT)
        self.path_rot_var     = tk.DoubleVar(value=PATH_ROT_DEFAULT)
        self.path_curve_var   = tk.DoubleVar(value=PATH_CURVE_DEFAULT)

        self.shore_chance_var = tk.DoubleVar(value=SHORE_CHANCE_DEFAULT)
    ...
    def create_widgets(self):
        ...
        ttk.Button(self, text="Rivers/Paths…", command=self.open_flow_editor).grid(row=13, column=5, padx=5, pady=5)
    ...
    def update_preview(self):
        ...
        terrain_img, veg_img = generate_terrain_and_vegetation(
            ...
            rivers=self.get_river_params(),
            paths=self.get_path_params(),
            shore_chance=self.shore_chance_var.get(),
        )
    ...
    def open_flow_editor(self):
        win = tk.Toplevel(self)
        win.title("Rivers, Paths & Shores")

        def add_slider(row, label, var, frm, to, res=0.001, fmt="{:.3f}"):
            ttk.Label(win, text=label).grid(row=row, column=0, sticky="e", padx=6, pady=4)
            s = ttk.Scale(win, from_=frm, to=to, orient="horizontal",
                          variable=var, command=lambda v: self.update_preview())
            s.grid(row=row, column=1, sticky="we", padx=6)
            val = ttk.Label(win, text=fmt.format(var.get()))
            val.grid(row=row, column=2, sticky="w", padx=6)

            def on_change(*_):
                val.config(text=fmt.format(var.get()))
                # keep min <= max
                if var is self.riv_minf_var and self.riv_minf_var.get() > self.riv_maxf_var.get():
                    self.riv_maxf_var.set(self.riv_minf_var.get())
                if var is self.riv_maxf_var and self.riv_maxf_var.get() < self.riv_minf_var.get():
                    self.riv_minf_var.set(self.riv_maxf_var.get())
        ...
        r = 0
        ttk.Label(win, text="RIVERS", font=("Arial", 10, "bold")).grid(row=r, column=0, sticky="w", pady=(8,2)); r+=1
        add_slider(r, "Scale",          self.riv_scale_var,   30, 180, 0.5, "{:.1f}"); r+=1
        add_slider(r, "Base Thickness", self.riv_band_var,   0.004, 0.030); r+=1
        add_slider(r, "Min Factor",     self.riv_minf_var,    0.2,  2.0, 0.001); r+=1
        add_slider(r, "Max Factor",     self.riv_maxf_var,    0.3,  3.0, 0.001); r+=1
        add_slider(r, "Rotation (deg)", self.riv_rot_var,   -90.0, 720.0, 0.5, "{:.1f}"); r+=1
        add_slider(r, "Curve Strength", self.riv_curve_var,    0.0, 720.0, 0.5, "{:.1f}"); r+=1
        add_slider(r, "Amount (×band)", self.riv_amount_var,   0.3,  2.5, 0.01, "{:.2f}"); r+=2

        ttk.Label(win, text="PATHS", font=("Arial", 10, "bold")).grid(row=r, column=0, sticky="w", pady=(8,2)); r+=1
        add_slider(r, "Scale",          self.path_scale_var,  25, 140, 0.5, "{:.1f}"); r+=1
        add_slider(r, "Thickness",      self.path_band_var, 0.004, 0.030); r+=1
        add_slider(r, "Rotation (deg)", self.path_rot_var,  -90.0, 720.0, 0.5, "{:.1f}"); r+=1
        add_slider(r, "Curve Strength", self.path_curve_var,   0.0, 720.0, 0.5, "{:.1f}"); r+=2

        ttk.Label(win, text="SHORELINES", font=("Arial", 10, "bold")).grid(row=r, column=0, sticky="w", pady=(8,2)); r+=1
        add_slider(r, "Shore Chance",   self.shore_chance_var, 0.0, 0.8, 0.01, "{:.2f}"); r+=1
    ...
    def get_river_params(self):
        if self.riv_minf_var.get() > self.riv_maxf_var.get():
            self.riv_maxf_var.set(self.riv_minf_var.get())
        return {
            "scale": self.riv_scale_var.get(),
            "base_band": self.riv_band_var.get(),
            "min_factor": self.riv_minf_var.get(),
            "max_factor": self.riv_maxf_var.get(),
            "rotation_deg": self.riv_rot_var.get(),
            "curve_strength": self.riv_curve_var.get(),
            "amount": self.riv_amount_var.get(),
        }

    def get_path_params(self):
        return {
            "scale": self.path_scale_var.get(),
            "band": self.path_band_var.get(),
            "rotation_deg": self.path_rot_var.get(),
            "curve_strength": self.path_curve_var.get(),
        }
...
# ====== POST-PROCESS MASKS ======
def apply_river_mask(
    img,
    noise_width,
    noise_height,
    seed,
    river_color=(0, 138, 255),
    river_scale=RIV_SCALE_DEFAULT,
    base_band=RIV_BAND_DEFAULT,
    min_factor=RIV_MINF_DEFAULT,
    max_factor=RIV_MAXF_DEFAULT,
    rotation_deg=RIV_ROT_DEFAULT,
    curve_strength=RIV_CURVE_DEFAULT,
    amount=RIV_AMOUNT_DEFAULT,
):
    """
    Domain-warped, rotated "stripe" noise -> thin, meandering rivers.
    """
    width, height = img.size
    pixels = img.load()

    base = (seed or 0) % 1024
    thick_base = ((seed or 0) + 123) % 1024
    warp_base = ((seed or 0) + 777) % 1024

    th = math.radians(rotation_deg)
    cos_t, sin_t = math.cos(th), math.sin(th)

    inv = 1.0 / river_scale
    warp_scale = river_scale * 2.0

    for x in range(width):
        for y in range(height):
            # domain warp
            wx = x + curve_strength * noise.pnoise2(x/warp_scale, y/warp_scale, octaves=2, persistence=0.5, lacunarity=2.0, base=warp_base)
            wy = y + curve_strength * noise.pnoise2((x+100)/warp_scale, (y-50)/warp_scale, octaves=2, persistence=0.5, lacunarity=2.0, base=warp_base+11)

            # rotate
            rx = (wx * cos_t - wy * sin_t) * inv
            ry = (wx * sin_t + wy * cos_t) * inv

            # stripe
            v = noise.pnoise2(rx, ry, octaves=4, persistence=0.5, lacunarity=2.0, base=base)
            v = abs(v)  # 0..1

            # thickness modulation
            tx = x / (river_scale * 0.6)
            ty = y / (river_scale * 0.6)
            t = noise.pnoise2(tx, ty, octaves=2, persistence=0.5, lacunarity=2.0, base=thick_base)
            t = (t + 1) * 0.5
            thick_mult = min_factor + (max_factor - min_factor) * t

            band = base_band * thick_mult * amount

            if v < band:
                pixels[x, y] = river_color

    return img


def apply_dirt_shoreline(
    img,
    terrain_normalized,
    water_color=(0, 138, 255),
    shore_color=(120, 70, 20),
    chance=0.25,
):
    """
    Add sand/dirt around any water-colored pixel (lakes or rivers).
    """
    width, height = img.size
    pixels = img.load()
    ...
    for x in range(width):
        for y in range(height):
            if pixels[x, y] == water_color:
                for dx, dy in neighbors:
                    nx = x + dx
                    ny = y + dy
                    if 0 <= nx < width and 0 <= ny < height:
                        if pixels[nx, ny] != water_color:
                            if random.random() < chance:
                                pixels[nx, ny] = shore_color
    return img
...
def apply_path_mask(
    ...
    path_scale=PATH_SCALE_DEFAULT,
    path_band=PATH_BAND_DEFAULT,
    rotation_deg=PATH_ROT_DEFAULT,
    curve_strength=PATH_CURVE_DEFAULT,
    skip_water_color=(0, 138, 255),
):
    """
    Rotated + domain-warped stripe noise -> dirt paths.
    """
    width, height = img.size
    pixels = img.load()

    base = (seed or 0) % 1024
    warp_base = ((seed or 0) + 991) % 1024

    th = math.radians(rotation_deg)
    cos_t, sin_t = math.cos(th), math.sin(th)

    inv = 1.0 / path_scale
    warp_scale = path_scale * 2.0

    for x in range(width):
        for y in range(height):
            if pixels[x, y] == skip_water_color:
                continue

            wx = x + curve_strength * noise.pnoise2(x/warp_scale, y/warp_scale, octaves=2, persistence=0.5, lacunarity=2.0, base=warp_base)
            wy = y + curve_strength * noise.pnoise2((x+33)/warp_scale, (y+77)/warp_scale, octaves=2, persistence=0.5, lacunarity=2.0, base=warp_base+17)

            rx = (wx * cos_t - wy * sin_t) * inv
            ry = (wx * sin_t + wy * cos_t) * inv

            v = abs(noise.pnoise2(rx, ry, octaves=3, persistence=0.5, lacunarity=2.0, base=base))
...
def generate_terrain_and_vegetation(width, height, scale, octaves, persistence, lacunarity,
                                    ...
                                    rivers=None, paths=None, shore_chance=None):
    # base terrain
    ...
    # colors
    terrain_layers = layer_config["terrain"]["layers"]
    water_col = _get_color(terrain_layers, "water")
    dirt_col  = _get_color(terrain_layers, "dirt")
    sand_col  = _get_color(terrain_layers, "sand")

    rivers = rivers or {}
    paths  = paths or {}
    if shore_chance is None:
        shore_chance = SHORE_CHANCE_DEFAULT

    # 1) rivers
    terrain_img = apply_river_mask(
        terrain_img,
        width, height,
        seed=(seed or 0) + 555,
        river_color=water_col,
        river_scale=rivers.get("scale", RIV_SCALE_DEFAULT),
        base_band=rivers.get("base_band", RIV_BAND_DEFAULT),
        min_factor=rivers.get("min_factor", RIV_MINF_DEFAULT),
        max_factor=rivers.get("max_factor", RIV_MAXF_DEFAULT),
        rotation_deg=rivers.get("rotation_deg", RIV_ROT_DEFAULT),
        curve_strength=rivers.get("curve_strength", RIV_CURVE_DEFAULT),
        amount=rivers.get("amount", RIV_AMOUNT_DEFAULT),
    )

    # 2) shoreline
    terrain_img = apply_dirt_shoreline(
        terrain_img,
        terrain_norm,
        water_color=water_col,
        shore_color=dirt_col,
        chance=shore_chance,
    )

    # 3) paths
    terrain_img = apply_path_mask(
        terrain_img,
        seed=(seed or 0) + 999,
        path_color=dirt_col,
        path_scale=paths.get("scale", PATH_SCALE_DEFAULT),
        path_band=paths.get("band", PATH_BAND_DEFAULT),
        rotation_deg=paths.get("rotation_deg", PATH_ROT_DEFAULT),
        curve_strength=paths.get("curve_strength", PATH_CURVE_DEFAULT),
        skip_water_color=water_col,
    )
